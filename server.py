...
class MyResource(resource.Resource):
    ...
    def __registration(self, request):
        # generated server uuid for client
        server_uuid = str(uuid.uuid4())

        # create row in db with client uuid and server uuid
        db.insert_registration(self.params[CLIENT_UUID], server_uuid, datetime.now())

        # send server uuid in headers to client
        request.setHeader(SERVER_UUID_HEADER, server_uuid)
        return ''.encode('utf-8')


    def __authentication(self, request):
        # select row drom db with server uuid and client uuid
        ...
        # if find row: send to client server and client uuids in headers
